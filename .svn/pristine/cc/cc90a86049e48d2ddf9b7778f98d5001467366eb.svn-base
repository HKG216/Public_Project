...
$(document).ready(function () {
    ...
    CKEDITOR.replace("editor_clinic_remarks", {
        on: {
            ...
            'instanceReady': function (evt) {
                evt.editor.resize("100%", $(".test1").height() - 30);
            },
            'change': function (evt) {
    ...
    $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
        if (e.target.hash == "#Soap_tab") {
            ...
            var rr = $(".test1").height();
            CKEDITOR.instances.editor_clinic_remarks.resize("100%", $(".test1").height() - 36);
            CKEDITOR.instances.editor_managment.resize("100%", $(".test2").height() - 36);
...
function fn_SaveEmptySoapData(dfd, status) {
    var inhospid = $('#patientInhospid').val();
    var chartno = $('#patientChartNo').val();
