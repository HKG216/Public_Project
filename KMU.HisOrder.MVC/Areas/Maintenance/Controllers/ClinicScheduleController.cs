...
namespace KMU.HisOrder.MVC.Areas.Maintenance.Controllers
{
    ...
    public class ClinicScheduleController : Controller
    {
        ...
        public string ModifySchedule(string Week, string Noon, string Room, string Dpt, string DptName, string Doctor, string DoctorName, string openFlag, string remark,string action)
        {
            #region Variable Setting
            ...
            using (ClinicScheduleService service = new ClinicScheduleService(_context))
            {
                result = service.ModifyScheduleData(Week, Noon, Room, Dpt, DptName, Doctor, DoctorName, openFlag, remark, checklogin.EMPCODE, action, EnumClass.DisplayLanguage.English);
            }
