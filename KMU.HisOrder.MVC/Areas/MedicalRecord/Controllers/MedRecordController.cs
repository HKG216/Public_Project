...
namespace KMU.HisOrder.MVC.Areas.MedicalRecord.Controllers
{
    ...
    public class MedRecordController : Controller
    {
        ...
        [HttpPost]
        public IActionResult AddAddress(KmuCoderef coderef)
        {
            ...
            if (coderef.RefName != null)
            {
                ...
                var lastId = _context.KmuCoderefs.Where(l => l.RefCodetype == "Chart_Area").Max(l => l.RefId);
                var ls = Convert.ToInt32(lastId + 1);
                coderef.RefId = Convert.ToString(ls);
