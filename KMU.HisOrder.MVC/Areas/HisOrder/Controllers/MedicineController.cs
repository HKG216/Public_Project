...
namespace KMU.HisOrder.MVC.Areas.HisOrder.Controllers
{
    ...
    public class MedicineController : Controller
    {
        ...
        public MedicineController(KMUContext context, IMapper mapper, ILogger<MedicineController> logger, ILoggerFactory loggerFactory)
        {
            _context = context;
            _mapper = mapper;
        ...
        private string InsertMedicineByObject(Hisorderplan inOrder, GlobalVariableDTO inGrv, string inStatus, bool doSaveChange = true)
        {
            ...
            catch (SqlException sql)
            {
                ...
                _logger.LogError("Sql Exception: {exception} PatientId: {patientdi} DoctorId: {doctorid} date: {date}", sql.Message, inGrv.Patient.RegPatientId, inGrv.Login.EMPCODE, DateTime.Now);

                return JsonConvert.SerializeObject(new ResultDTO() { isSuccess = false });
            }
            catch (Exception EX)
            {
                _logger.LogError("Exception: {exception} patientId: {patientId} doctorId: {doctorId} Date: {date}" + EX.Message, inGrv.Patient.RegPatientId, inGrv.Login.EMPCODE, DateTime.Now);
        ...
        private string UpdateMedByObject(Hisorderplan inOrder, GlobalVariableDTO inGrv, string inStatus)
        {
            ...
            {
                _logger.LogError("Exception: {exception} PatientId: {patientdi} DoctorId: {doctorid} Date {date}", EX.Message, inGrv.Patient.RegPatientId, inGrv.Login.EMPCODE, DateTime.Now);
